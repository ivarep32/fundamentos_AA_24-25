...
datos_L <- na.omit(datos_L)

# analisis estandarizado con pca
pca <- prcomp(datos_L, scale. = TRUE)
summary(pca)
# PC1 aporta 34.5 % de la variabilidad total. combina de forma varias variables
# del último periodo, concretamente las relacionadas con temperatura y radiación solar,

# PC2 aporta 21.2 %, acumulando 55.7 %. normalmente captura una dirección ortogonal
# a PC1.

# PC3 aporta 12.9 %, acumulando 68.6 %. recoge variaciones más sutiles.

# PC4 aporta 10.1 % ,acumulando 78.7 %.
# puede estar capturando efectos mas especificos como relaciones no lineales


#añadimos mes
scores <- data.frame(pca$x[, 1:2], mes = KorTemp$mes)

# puntuaciones de las dos primeras variables por mes
library(ggplot2) #para la grafica

ggplot(scores, aes(x = PC1, y = PC2, color = mes)) +
  geom_point(alpha = 0.6) +
  theme_minimal() +
  labs(title = "puntuaciones coloreadas por mes")
# las 4 primeras componentes principales explican el 78.72% de la variablidad
# (Cumulative Proportion  0.3452 0.5570 0.6862 0.7872) representando asi la amplia
# mayoria de los datos
# El grafico de puntuaciones muestra como varían entre los meses de julio
# (azul) y agosto (rojo) viendo que en agosto la nube es mucho mas dispersa
# y tiende a valores menores tanto de PC1 como de PC2